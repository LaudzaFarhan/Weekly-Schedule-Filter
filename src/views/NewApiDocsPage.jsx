...
const RESOURCES = [
  ...
  },
  {
    ...
    path: '/api/new/crm',
    table: 'new_crm_leads',
    description: 'CRM pipeline leads (trial interest and follow-ups). Also available at /api/crm.',
    fields: 'name, phone, message, status, branch, trialDate, notes, attendanceStatus, paymentStatus',
    required: 'name, phone',
    methods: [
      { m: 'GET', d: 'List leads — filters: ?id=&search=&status=&branch=&metric=profiling|scheduled|junk&limit=' },
      { m: 'POST', d: 'Create lead record' },
      { m: 'PUT', d: 'Update lead — body must include id, phone, or name' },
      { m: 'PATCH', d: 'Partial update of lead' },
      { m: 'DELETE', d: 'Delete lead by ?id=' },
    ],
    example: {
  ...
  {
    name: 'CRM Performance & Analytics',
    path: '/api/new/crm/performance',
    table: 'new_crm_leads',
    description: 'Pre-computed & dynamic analytics for Weekly & Monthly Performance: 4 metrics (Leads, Profiling, Trial Scheduled, Junk Leads), 12-month trends, and cross-branch comparative breakdown. Also available at /api/crm/performance.',
    fields: 'totals, months[], branchTotals[], availableBranches[], allBranchesTotalLeads',
    required: 'none (GET query parameters)',
    methods: [
      { m: 'GET', d: 'Analytics — filters: ?year=2026&branch=all|Bekasi&month=2026-08|8&includeLeads=true|false' },
    ],
    example: {
      year: '2026',
      branch: 'all',
  ...
  {
    name: 'CRM Inbound Webhook',
    path: '/api/new/crm/webhook',
    table: 'new_crm_leads',
    description: 'Inbound webhook for WhatsApp bot (Qontak), Facebook/Meta Lead Ads, Zapier, and web forms. Automatically extracts parent/child names and age, and runs classification. Also available at /api/crm/webhook.',
    fields: 'name, parent_name, child_name, phone, age, branch, message, trial_date, status, notes',
    required: 'name (or parent_name/child_name), phone',
    methods: [
      { m: 'POST', d: 'Ingest lead from external webhook with automatic profiling & spam detection' },
    ],
    example: {
      ...
      phone: '08123456789',
      branch: 'Bekasi',
      message: 'Inquiring about Saturday trial class',
    },
  },
  {
    name: 'Operationals',
